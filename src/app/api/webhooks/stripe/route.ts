...
export async function POST(request: Request) {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      console.log("session metadata is", session.metadata);
      // Extract metadata
      const userId = session.metadata?.userId;
      const planType = session.metadata?.planType;
      const snippetLimit = session.metadata?.snippetLimit
        ? Number.parseInt(session.metadata.snippetLimit)
        : 0;
      ...
      if (
        ...
        subscriptionData.snippet_limit > PLANS.FREE.snippetLimit
      ) {
        newSnippetLimit =
          subscriptionData.snippet_limit +
          snippetLimit -
          PLANS[subscriptionData.plan_type.toUpperCase() as keyof typeof PLANS]
            .snippetLimit;
      }
      ...
      await supabaseAdmin
        ...
        .update({
          ...
          stripe_price_id: session.amount_total
